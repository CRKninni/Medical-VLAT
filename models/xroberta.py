...
class RobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if soft_labels is not None:
            ...
            min_seq_length = min(labels.size(1), loss_distill.size(1))
            labels = labels[:, :min_seq_length]
            loss_distill = loss_distill[:, :min_seq_length]
            
            # Apply the mask
            valid_loss = loss_distill[labels != -100]
            loss_distill = valid_loss.mean()
            
            # Combine with masked LM loss
